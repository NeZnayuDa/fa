...
function renderProducts(list) {
    ...
    if (!list || list.length === 0) {
        filmContainer.innerHTML = '<p class="no-results">No results found</p>';
        return;
    }

    list.forEach(film => {
        const filmElement = document.createElement('div');
        filmElement.className = 'product';
        filmElement.innerHTML = `
            <div>
                <img src="${film.image}" alt="${film.title}" style="width: 200px; height: auto;" />
                <h2>${film.title}</h2>
                <p>ID: ${film.id}</p>
